...
int StepMotorControl(int CURRENT_SPEED,int TARGET_SPEED){
    ...
    // }
     ...
     if(MOTOR.CURRENT_PWM_DUTY < -max_pwm) MOTOR.CURRENT_PWM_DUTY = -max_pwm;
    LCD("I",MOTOR.CURRENT_PWM_DUTY,6);
    if(MOTOR.CURRENT_PWM_DUTY>=0) STEP_MOTOR_FORWARD(&MOTOR,MOTOR.CURRENT_PWM_DUTY);
    if(MOTOR.CURRENT_PWM_DUTY<0) STEP_MOTOR_BACK(&MOTOR,-MOTOR.CURRENT_PWM_DUTY);
